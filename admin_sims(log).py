...
logging.basicConfig(level=logging.INFO,
                    filename="sims.log", filemode="a",format="We have next logging message: ""%(asctime)s:%(levelname)s-%(message)s"
                    )
class Admin:
    ...
    def vacation(self):
        if self.has_job == 0:
            ...
            logging.info("To have vacation I need to work more")
            self.work()
            pass
        elif self.has_job == 1 and self.money < 2500:
            print("I don't have enough money for this, so I just gonna work")
            logging.info("I don't have enough money for this, so I just gonna work")
            self.work()
            pass
        else:
            self.money -= 2500
    ...
    def restaurant(self):
        if self.money >= 1000 and self.has_job == 1:
            ...
            logging.info("I wanna eat something delicious")
            self.money -= 1000
            self.cheerfulness += 10
            self.money += self.company.salary_chill
        elif self.has_job == 0:
            print("Let's eat some pizza")
            logging.info("Let's eat some pizza")
            self.money -= 30
            self.cheerfulness += 8
        else:
            print("I don't have enough money for this, so I need to work")
            logging.info("I don't have enough money for this, so I need to work")
            self.work()
            pass
    def chill(self):
        self.cheerfulness += 7
        ...
        if pay_or_not == 1 and self.has_job == 0:
            ...
            logging.info("So I gonna eat some street food ")
            self.money -= 15
            self.cheerfulness += 3
        elif pay_or_not == 1 and self.has_job == 1:
            print("Let's go shopping")
            logging.info("Let's go shopping")
            self.money -= 200
            self.cheerfulness += 10
            self.money += self.company.salary_chill
        else:
    ...
    def day_indexes(self, day):
        ...
        if self.has_job == 1:
            ...
            logging.info(f"Cheerfulness - {self.cheerfulness}")
            company_indexes = "Company indexes"
            print(f"{company_indexes:^50}", "\n")
            logging.warning(f"{company_indexes:^50}")
            print(f"Company - {self.company.company}")
            logging.info(f"Company - {self.company.company}")
            ...
            logging.info(f"Popularity - {round(self.company.popularity, 2)}")
        else:
            print(f"Money - {self.money}")
            logging.info(f"Money - {self.money}")
            if self.cheerfulness > 100:
    ...
    def live(self, day):
        ...
        if self.money <= 0:
            ...
            logging.info("I need to start working immediately!")
            self.work()
        elif self.cheerfulness <= 15:
            print("I need to chill!")
            logging.info("I need to chill!")
            self.chill()
        elif self.has_job == 1:
            if self.company.popularity <= 0:
                print("I don't want to lose my company, so I need to work")
                logging.info("I don't want to lose my company, so I need to work")
                self.work()

        cube = random.randint(1, 4)
        if cube == 1:
            print("Let's earn money baby")
            logging.info("Let's earn money baby")
            self.work()
        if cube == 2:
            print("Today I wanna chill")
            logging.info("Today I wanna chill")
            self.chill()
        if cube == 3:
            self.restaurant()
